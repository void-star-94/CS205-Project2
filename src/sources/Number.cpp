...
namespace void_star {
    ...
    inline bool abs_less(const Number &a, const Number &b) {
        bool origin_a = a.sign, origin_b = b.sign;
        a.sign = POSITIVE, b.sign = POSITIVE;
        bool re = a <= b;
        a.sign = origin_a, b.sign = origin_b;
        return re;
    }
    ...
    Number::Number(const std::string_view &stringView) {
        nums.reserve(precision);
        u64 i = stringView.size(), k = stringView.size(), nz = 0;
        bool n = false;
        for (u64 j = 0; j < stringView.size(); ++j) {
            if (stringView[j] == 'e') {
                i = j;
            } else if (stringView[j] == '.') {
                k = j;
            } else if (stringView[j] != '0') {
                if (!n) {
                    n = true;
                    nz = j;
                }
        ...
        if (i == stringView.size() && k == stringView.size()) {
            for (u64 j = stringView.size() - 1; j > nz; --j) {
                nums.push_back(stringView[j] - '0');
            }
            nums.push_back(stringView[nz] - '0');
            power = 0;
        } else if (i == stringView.size()) {
            for (u64 j = stringView.size() - 1; j > nz; --j) {
                if (j == k)continue;
                nums.push_back(stringView[j] - '0');
            }
            nums.push_back(stringView[nz] - '0');
            power = -i64(stringView.size() - k - 1);
        } else if (k == stringView.size()) {
            std::string_view sv(stringView.begin() + i + 1, stringView.end());
            for (u64 j = i - 1; j > nz; --j) {
                nums.push_back(stringView[j] - '0');
            }
            nums.push_back(stringView[nz] - '0');
            power = strtoll(std::string(sv).c_str(), nullptr, 10);
        } else {
            std::string_view sv(stringView.begin() + i + 1, stringView.end());
            power = strtoll(std::string(sv).c_str(), nullptr, 10);
            for (u64 j = i - 1; j > nz; --j) {
    ...
    Number Number::operator/(const Number &other) const {//check before use
        ...
        a.power = power + pad;
        re.sign = !(sign xor other.sign);
        re.nums.resize(pre, 0);
        b.power = i64(a.power + a.nums.size() - b.nums.size());
        bool extra = false;
        i64 i;
        for (i = 0; i < pre; ++i, b.power--) {
            if (b > a) {
                if (i) {
                    continue;
                }
...
//                std::cout << a << "\n";
        ...
        re.power = power - other.power - pad - extra + 1;
        re.squeeze();
        return re;
    }
    ...
    std::pair<Number, Number> Number::operator%(const Number &other) const {//check before use
        ...
        if (l.isZero())
            return {{},
                    {}};
        i64 tmp = l.power + i64(l.nums.size());
        if (tmp <= 0) {
            return {{}, *this};
        }
        Number re = l;
        re.nums.erase(re.nums.begin(), re.nums.begin() - l.power);
        re.power = 0;
        re.squeeze();
        re = re.sign ? re : --re;

        Number r = (*this) - (re * other);
        r.squeeze();
        return {l, r};
    }
